...
7. Update `version` and `pyproject.toml` with the new version.
8. Run tests to verify nothing is broken: `pytest` (or your test runner).
9. Build the package: `python -m build`.
10. In `CHANGELOG.md`, **replace the `## [Unreleased]` heading with `##
...
12. Create a git tag: `git tag vX.X.X`.
13. Push the tag: `git push origin vX.X.X`.
14. (Optional) Publish to PyPI: `twine upload dist/*`.
